var D=Object.defineProperty;var B=(a,e,t)=>e in a?D(a,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):a[e]=t;var r=(a,e,t)=>(B(a,typeof e!="symbol"?e+"":e,t),t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const i of o.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function t(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(s){if(s.ep)return;s.ep=!0;const o=t(s);fetch(s.href,o)}})();var b=(a=>(a.Default="default",a.Page="page",a.NotFound="not-found",a))(b||{});class k{constructor(e,t,n){r(this,"url");r(this,"type");r(this,"routing");this.url=e,this.type=t,this.routing=n}}class $ extends HTMLElement{constructor(){super();r(this,"shadowRoot");r(this,"routes",[]);this.shadowRoot=this.attachShadow({mode:"closed"})}connectedCallback(){window.addEventListener("hashchange",()=>{this.checkRoute()})}addRoutes(t){this.routes=[...t],this.checkRoute()}checkRoute(){const t=window.location.hash.slice(2);this.changeRoute(t)}changeRoute(t){if(t){const n=this.routes.findIndex(s=>s.url===t);this.shadowRoot.innerHTML=this.routes[n]?this.routes[n].routing():this.sendNotFound()}else{const n=this.routes.filter(s=>s.type===b.Default);n?window.location.hash="#/"+n[0].url:this.sendNotFound()}}sendNotFound(){const t=this.routes.filter(n=>n.type===b.NotFound);return t.length===0||(window.location.hash="#/"+t[0].url,this.changeRoute(t[0].url)),"404: Not found"}}customElements.define("app-router",$);class A extends HTMLElement{constructor(){super();r(this,"shadowRoot");this.shadowRoot=this.attachShadow({mode:"closed"})}connectedCallback(){this.render()}render(){this.shadowRoot.innerHTML=`
            <div class="loader">
            ...
            `}}customElements.define("app-loader",A);class C{constructor(e,t){r(this,"color");r(this,"opacity");this.color=e,this.opacity=t}static createEmpty(){return new C("#008000",1)}}class H{constructor(e,t,n){r(this,"propertyName");r(this,"displayName");r(this,"type");this.propertyName=e,this.displayName=t,this.type=n}static createEmpty(){return new H("","","string")}}var E=(a=>(a.String="string",a.Image="image",a.Number="number",a))(E||{});class L{constructor(e,t,n,s,o,i){r(this,"name");r(this,"layer");r(this,"url");r(this,"style");r(this,"tags");r(this,"relevantProperties");this.name=e,this.layer=t,this.url=n,this.style=s,this.tags=o,this.relevantProperties=i}static createEmpty(){return new L("","","",C.createEmpty(),[],[H.createEmpty()])}}const d=class d{constructor(){r(this,"CATEGORIES_URL","./json/categories.json");r(this,"_data",{categories:[]});if(d._instance)return d._instance;d._instance=this}static get instance(){return d._instance||(d._instance=new d),d._instance}get data(){return this._data}set data(e){this._data=e}async getData(){if(this.data.categories.length!==0)return this._data;{let e=await this.fetchAppData(this.CATEGORIES_URL);return e=this.parseData(e),this.data=e,e}}async fetchAppData(e){try{const t=await fetch(e).then(s=>s.json()),n=await Promise.all(t.categories.map(async s=>{const o=await Promise.all(s.groups.map(async i=>{if(typeof i=="string")try{const c=await fetch(i);if(c.ok)return c.json();throw new Error("Errore durante il recupero dei dati.")}catch(c){return console.error(c),null}else return i}));return s.groups=o,s}));return{...t,categories:n}}catch(t){throw console.error("Errore durante il recupero dei dati JSON.",t),t}}parseData(e){return{categories:e.categories.map(n=>({name:n.name,groups:n.groups.map(s=>this.parseGroup(s))}))}}parseGroup(e){return Array.isArray(e)?e:{name:e.name,layers:e.layers.map(t=>this.parseLayer(t))}}parseLayer(e){return new L(e.name,e.layer,e.layer_url_wfs,new C(e.style.color,parseFloat(e.style.opacity)),e.tags,e.relevant_properties.map(t=>{let n=H.createEmpty();switch(n.displayName=t.display_name,n.propertyName=t.property_name,t.type){case"image":n.type=E.Image;break;case"number":n.type=E.Number;break;default:n.type=E.String;break}return n}))}getAllTags(e){let t=[];return e.categories.map(s=>{s.groups.map(o=>{typeof o!="string"&&o.layers.map(i=>{i.tags.map(c=>{t.push(c)})})})}),[...new Set(t)]}filterLayersByTags(e){let t=[];return e.forEach(s=>{this.filterLayersByTag(s).forEach(i=>t.push(i))}),[...new Set(t)]}filterLayersByTag(e){let t=[];return t=this.data.categories.flatMap(n=>n.groups.flatMap(s=>typeof s=="string"?[L.createEmpty()]:s.layers.filter(o=>o.tags.some(i=>i.includes(e))))),t}};r(d,"_instance");let w=d;const p=class p{constructor(){r(this,"_activeLayers",[]);if(p._instance)return p._instance;p._instance=this}static get instance(){return p._instance||(p._instance=new p),p._instance}get activeLayers(){return this._activeLayers}set activeLayers(e){this._activeLayers=e,localStorage.setItem("layers",JSON.stringify(this.activeLayers))}getSavedLayers(){const e=localStorage.getItem("layers");if(!e)return;const t=JSON.parse(e);let n=[];n=t.map(s=>this.parseLayer(s)),this._activeLayers=n}parseLayer(e){return new L(e.name,e.layer,e.url=e.url,new C(e.style.color,e.style.opacity),e.tags,e.relevantProperties.map(t=>{let n=H.createEmpty();switch(n.displayName=t.displayName,n.propertyName=t.propertyName,t.type){case"image":n.type=E.Image;break;case"number":n.type=E.Number;break;default:n.type=E.String;break}return n}))}};r(p,"_instance");let R=p;class F extends HTMLButtonElement{constructor(){super();r(this,"_tag","")}get tag(){return this._tag}set tag(t){this._tag=t}connectedCallback(){this.setup()}setup(){this.addEventListener("click",()=>{this.dispatchEvent(new CustomEvent("tag-selected",{detail:{tag:this.tag}}))})}}customElements.define("app-tag-chip",F,{extends:"button"});class O extends HTMLElement{constructor(){super();r(this,"shadowRoot");r(this,"_tags",[]);r(this,"_currentPageTags",[]);r(this,"_currentPage",0);r(this,"_tagsPerPage",8);r(this,"handleCheckbox",t=>{const n=t.target;this.dispatchEvent(new CustomEvent("tag-selected",{detail:{tag:n.tag}}))});this.shadowRoot=this.attachShadow({mode:"closed"})}get tags(){return this._tags}set tags(t){this._tags=t,this.connectedCallback()}get currentPageTags(){return this._currentPageTags}set currentPageTags(t){this._currentPageTags=t}get currentPage(){return this._currentPage}set currentPage(t){this._currentPage=t}get tagsPerPage(){return this._tagsPerPage}set tagsPerPage(t){this._tagsPerPage=t}connectedCallback(){this.render(),this.tags.length!==0&&(this.paginateTags(),this.setup())}render(){this.shadowRoot.innerHTML=`
            ...
            `}setup(){const e=this.querySelector("button");e&&e.addEventListener("click",()=>this.close())}}customElements.define("app-menu",U,{extends:"dialog"});class G extends HTMLElement{constructor(){super();r(this,"shadowRoot");r(this,"_poi",new v);this.shadowRoot=this.attachShadow({mode:"closed"})}get poi(){return this._poi}set poi(t){this._poi=t}connectedCallback(){this.render(),this.setup()}render(){this.shadowRoot.innerHTML=`
            <div class="poi-card">
                <h3>${this.poi.name}</h3>
            ...
            `}setup(){this.handleSlider()}handleSlider(){const t=this.shadowRoot.querySelector('input[type="range"]'),n=Array.from(this.shadowRoot.querySelectorAll("#font-size-values option")).map(s=>parseFloat(s.value));!t||!n||t.addEventListener("input",()=>{let s=parseFloat(t.value);s=n.reduce((i,c)=>Math.abs(c-s)<Math.abs(i-s)?c:i),t.value=s.toString(),this.fontSize=s})}update(){const t=this.shadowRoot.querySelector('input[type="range"]');t&&(t.value=this.fontSize.toString())}}customElements.define("app-settings-font-size",Y);class K extends HTMLElement{constructor(){super();r(this,"shadowRoot");r(this,"_letterSpace",0);this.shadowRoot=this.attachShadow({mode:"closed"})}get letterSpace(){return this._letterSpace}set letterSpace(t){this._letterSpace=t,this.update(),this.dispatchEvent(new CustomEvent("letter-space-updated",{detail:{letterSpace:this.letterSpace}}))}connectedCallback(){this.render(),this.setup()}render(){this.shadowRoot.innerHTML=`
            <div class="component">
            ...
            `}setup(){this.handleSlider()}handleSlider(){const t=this.shadowRoot.querySelector('input[type="range"]'),n=Array.from(this.shadowRoot.querySelectorAll("#letter-space-values option")).map(s=>parseFloat(s.value));!t||!n||t.addEventListener("input",()=>{let s=parseFloat(t.value);s=n.reduce((i,c)=>Math.abs(c-s)<Math.abs(i-s)?c:i),t.value=s.toString(),this.letterSpace=s/100})}update(){const t=this.shadowRoot.querySelector('input[type="range"]');t&&(t.value=(this.letterSpace*100).toString())}}customElements.define("app-settings-letter-space",K);class Q extends HTMLElement{constructor(){super();r(this,"shadowRoot");r(this,"_lineHeight",1.15);this.shadowRoot=this.attachShadow({mode:"closed"})}get lineHeight(){return this._lineHeight}set lineHeight(t){this._lineHeight=t,this.update(),this.dispatchEvent(new CustomEvent("line-height-updated",{detail:{lineHeight:this.lineHeight}}))}connectedCallback(){this.render(),this.setup()}render(){this.shadowRoot.innerHTML=`
            <div class="component">
                <h2>Altezza testo</h2>
            ...
            `}update(){const t=this.shadowRoot.querySelector(".test-text");t&&(t.style.setProperty("font-size",`${this.settings.fontSize}px`),t.style.setProperty("letter-spacing",`${this.settings.letterSpace}rem`),t.style.setProperty("line-height",this.settings.lineHeight+""))}}customElements.define("app-text-test",X);class Z extends HTMLElement{constructor(){super();r(this,"shadowRoot");r(this,"_settings",{...f.instance.settings});this.shadowRoot=this.attachShadow({mode:"closed"})}get settings(){return this._settings}set settings(t){this._settings=t}connectedCallback(){this.render(),this.setup(),this.update()}render(){this.shadowRoot.innerHTML=`
            <div class="page">
            ...
            `}setup(){const t=this.shadowRoot.querySelector("app-settings-contrast"),n=this.shadowRoot.querySelector("app-settings-font-size"),s=this.shadowRoot.querySelector("app-settings-letter-space"),o=this.shadowRoot.querySelector("app-settings-line-height"),i=this.shadowRoot.querySelector(".apply-btn");t&&n&&s&&o&&i&&(t.contrast=this.settings.contrast,n.fontSize=this.settings.fontSize,o.lineHeight=this.settings.lineHeight,s.letterSpace=this.settings.letterSpace,t.addEventListener("contrast-updated",c=>{this.settings.contrast=c.detail.contrast,f.instance.settings.contrast=this.settings.contrast,f.instance.setContrast()}),n.addEventListener("font-size-updated",c=>{this.settings.fontSize=c.detail.fontSize,this.update()}),s.addEventListener("letter-space-updated",c=>{this.settings.letterSpace=c.detail.letterSpace,this.update()}),o.addEventListener("line-height-updated",c=>{this.settings.lineHeight=c.detail.lineHeight,this.update()}),i.addEventListener("click",()=>{window.location.href="/#/around-you",f.instance.settings=this.settings}))}update(){const t=this.shadowRoot.querySelector("app-text-test");t&&(t.settings=this.settings)}}customElements.define("page-settings",Z);class tt extends HTMLElement{constructor(){super();r(this,"shadowRoot");r(this,"_poi",new v);this.shadowRoot=this.attachShadow({mode:"closed"})}get poi(){return this._poi}set poi(t){this._poi=t}connectedCallback(){M.instance.getSelectedPoi(),this.poi=M.instance.selectedPoi,this.render(),this.setup()}render(){this.shadowRoot.innerHTML=`
            <div class="page-poi">
                <h1>${this.poi.name}</h1>
                <p>${this.poi.name}</p>
            ...
            `}}customElements.define("page-custom-path",et);f.instance.getLocalStorageSettings();const nt=document.querySelector("app-router"),st=new k("categories",b.Default,()=>"<page-categories />"),rt=new k("around-you",b.Page,()=>"<page-around-you />"),ot=new k("settings",b.Page,()=>"<page-settings />"),at=new k("poi",b.Page,()=>"<page-poi />"),it=new k("custom-path",b.Page,()=>"<page-custom-path />"),ct=[st,rt,ot,at,it];nt.addRoutes(ct);
